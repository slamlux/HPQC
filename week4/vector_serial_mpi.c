...
int root_task(int uni_size, int num_arg, int my_vector[])
{
	...
	chunk = start = stop = 0;
	for (int my_rank = 1; my_rank < uni_size; my_rank++)
	{
		...
		// creates a vector variable
		int* vector_sub = malloc (chunk * sizeof(int));
		...
		MPI_Send(vector_sub, chunk, MPI_INT, my_rank, tag, MPI_COMM_WORLD);
...
void client_task(int my_rank, int num_arg, int uni_size, int my_vector[])
{
	...
	MPI_Recv(recv_message, chunk, MPI_INT, source, tag, MPI_COMM_WORLD, &status);


	// sums the vector
	int my_sum = sum_vector(recv_message, chunk);
	//printf("%d\n", my_sum);
