...
void MPI_user_sum(void *invector, void *outvector, int *len, MPI_Datatype *datatype);
...
int root_task(int uni_size, int num_arg, int my_vector[])
{
	...
	MPI_Op op;
	MPI_Op_create(&MPI_user_sum, 1, &op);
...
void client_task(int my_rank, int num_arg, int uni_size, int my_vector[])
{
	...
	int* vector = malloc (1 * sizeof(int));
	initialise_vector(vector, 1, 0);
	if (my_rank == uni_size-1)
	{
		stop = chunk + (num_arg % (uni_size-1)) ;
	}
...
void MPI_user_sum(void *invector, void *outvector, int *len, MPI_Datatype *datatype)
{
	...
	int *invecdble    = (int *) invector;
 	int *inoutvecdble = (int *) outvector;

	// sets the elements of the vector to the initial value
	inoutvecdble[0] += invecdble[0];
